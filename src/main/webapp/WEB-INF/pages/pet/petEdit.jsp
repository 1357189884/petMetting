...
<body>
	<form class="form-inline definewidth m20" action="index.html"
		...
		<button type="submit" class="btn btn-primary"
			onclick="formSubmit('update','_self');this.blur();">编辑</button>
		&nbsp;&nbsp;
		<button type="submit" class="btn btn-primary"
			onClick="formSubmit('heal','_self');this.blur();">健康</button>
		&nbsp;&nbsp;
		<button type="submit" class="btn btn-primary"
			onClick="formSubmit('noheal','_self');this.blur();">非健康</button>
		&nbsp;&nbsp;
				...
				<c:forEach items="${pets}" var="p" varStatus="status">
					<tr>
						...
						<td><c:if test="${p.health==1}">
								<a href="noheal?petId=${p.petId}"><font color="green">健康</font></a>
							</c:if> <c:if test="${p.health==0}">
								<a href="heal?petId=${p.petId}"><font color="red">非健康</font></a>
							</c:if></td>
